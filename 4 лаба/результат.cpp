...
void fatman_func(int id, int gluttony) {
    while (running && alive[id]) {
        ...
        // Ждем, пока можно есть
        while (running && alive[id] && (!fatmen_can_eat || current_fatman != id)) {
        ...
        if (dishes[id] >= gluttony) {
            ...
            if (eaten[id] >= MAX_NUGGETS_PER_FATMAN) {
                ...
                // Проверяем, все ли толстяки мертвы
        ...
        // Переходим к следующему толстяку
    ...
    while (running && day_counter < SIMULATION_DAYS) {
        ...
        if (cycles_counter >= cycles_per_day) {
            ...
            if (day_counter <= SIMULATION_DAYS) {
                ...
                // Если все толстяки мертвы, завершаем
                if (all_dead) {
                    cook_no_salary = true;
                    running = false;
        ...
        // Короткая пауза для симуляции работы
        lock.unlock();
...
int main() {
    ...
    // Эксперимент 1: Кука увольняют (мало готовит, много едят)
    run_experiment(180, 15, 5,
        "ЭКСПЕРИМЕНТ 1: КУКА УВОЛИЛИ",
        "Кука уволили");

    // Эксперимент 2: Кук без зарплаты (все толстяки самоуничтожаются до 5 дней)
    run_experiment(180, 210, 12,
        "ЭКСПЕРИМЕНТ 2: КУК БЕЗ ЗАРПЛАТЫ",
        "Кук без зарплаты");

    // Эксперимент 3: Кук увольняется сам (5 дней прошло, толстяки еще не наелись)
